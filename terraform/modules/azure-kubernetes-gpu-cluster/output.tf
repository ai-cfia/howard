output "cluster_name" {
  ...
  value       = azurerm_kubernetes_cluster.k8s-gpu.name
}

output "cluster_ca_certificate" {
  description = "Cluster certificate"
  value       = azurerm_kubernetes_cluster.k8s-gpu.kube_config[0].cluster_ca_certificate
}

output "cluster_principal_id" {
  description = "AKS cluster principal id"
  value       = azurerm_kubernetes_cluster.k8s-gpu.identity[0].principal_id
}
...
output "kv_identity_resource_id" {
  ...
  value       = azurerm_kubernetes_cluster.k8s-gpu.key_vault_secrets_provider[0].secret_identity[0].object_id
}
